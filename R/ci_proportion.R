...
#'
#' This function calculates confidence intervals for a population proportion. By default, "Clopper-Pearson" confidence intervals are calculated (via \code{stats::binom.test}). Further possibilities are "Wilson" and "Agresti-Coutts" intervals. Bootstrap confidence intervals (not recommended) are calculated by the package "resample". The default bootstrap type is "BootstrapT".
#'
...
#' n <- 1000
#' ci_proportion(x, n)
#' ci_proportion(x, n, type = "Agresti–Coull")
#' ci_proportion(x, n, type = "Agresti–Coull", probs = c(0.05, 1))
ci_proportion <- function(x, n = NULL, probs = c(0.025, 0.975),
  ...
  if (length(x) == 1L) {
    ...
    stop("x must either be the number of successes or a 0-1 vector.")
  }

  # Calculate CI
  if (type == "Clopper-Pearson") {
    cint <- as.numeric(binom.test(x, n, conf.level = diff(probs))$conf.int)
  } else if (type %in% c("Wilson", "Agresti–Coull")) {
    z <- qnorm(1 - if (is_symmetric(probs)) alpha / 2 else alpha)
    nt <- n + z^2
    pt <- (x + z^2 / 2) / nt
    if (type == "Wilson") {
    ...
    xx <- rep(0:1, times = c(n - x, x))
    if (boot_type == "bootstrapT") {
      S <- bootstrap(xx, statistic = c(mean = mean(xx), sd = se_proportion(xx)), R = R, seed = seed)
    } else {
      S <- bootstrap(xx, statistic = mean, R = R, seed = seed)
    }
