---
title: "Using the 'cis' Package"
author: "Michael Mayer"
date: "`r Sys.Date()`"
bibliography: "biblio.bib"
link-citations: true
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{outForest}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  warning = FALSE,
  message = FALSE
)
```

## Introduction

The aim of this vignette is to introduce the R package `cis` for calculating classic and bootstrap confidence intervals.

Confidence intervals for the following parameters are available:

- mean,

- proportion,

- variance and standard deviation,

- skewness and kurtosis,

- R-squared and the non-centrality parameter of the F distribution,

- Cramér's V and the non-centrality parameter of the chi-squared distribution,

- Pearson-, Spearman-, Kendall correlation coefficients

Many of the classic confidence intervals are described in Smithson [@smithson2003]. Bootstrap confidence intervals are described in Efron [@efron1939] and implemented by the `boot` package [@ripley2019].

## Installation

From CRAN:
```
install.packages("cis")
```

Latest version from github:
```
library(devtools)
install_github("mayer79/cis")
```

## Examples

```{r}
library(cis)

# Mean
ci_mean(1:100)
ci_mean(1:100, type = "bootstrap")

# Correlation
ci_cor(iris[1:2], method = "spearman", type = "bootstrap", R = 1000)

# Proportions
ci_proportion(10, n = 100, type = "Wilson")
ci_proportion(10, n = 100, type = "Clopper-Pearson")

# R-squared
fit <- lm(Sepal.Length ~ ., data = iris)
ci_rsquared(fit, probs = c(0.05, 1))

# Kurtosis
ci_kurtosis(1:100)
```

## References

