context("Confidence intervals for univariate parameters")
...
test_that("ci_sd works", {
  ...
  x <- rnorm(100)
  xp <- (x - mean(x)) / sd(x) * 8
  expect_equal(ci_sd(xp, probs = c(0, 0.95))$interval[2], 9.07, tol = 0.01)
})
...
test_that("ci_median works", {
  ...
  expect_equal(ci_median(x, type = "bootstrap", R = 249, seed = 2)$interval, c(7.758812, 17.000000), tol = 0.0001)
...
test_that("ci_proportion works", {
  ...
  expect_equal(ci_proportion(x, n, type = "bootstrap", R = 249, seed = 2)$interval, c(0.1115616, 0.35), tol = 0.0001)
  expect_equal(ci_proportion(x, n, type = "Wilson")$interval, c(0.1418712, 0.4019396), tol = 0.0001)
  expect_equal(ci_proportion(x, n, type = "Agresti-Coull")$interval, c(0.1401985, 0.4036123), tol = 0.0001)
